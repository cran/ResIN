...
Alternative score estimates that are based on partial pooling of item and individual level means are implemented with a simple heuristic approach, `heur_x`, weighted by relative item popularity, and `eb_x` which implements empirical Bayes shrinkage following Efron and Morris (1973). Either of the latter options are recommended if there are large discrepancies in item response popularity or if there is substantial missingness in the raw data.
